 ...
 * @version     SVN: \$Id: CO_SDOmaster.c 42 2013-07-09 11:14:12Z jani22 $
...
/* Private variable ----------------------------------------------------------*/
/* Private function ----------------------------------------------------------*/
static void CO_SDOclient_receive(void *object, CO_CANrxMsg_t *msg);
        ...
        uint8_t                 blockEnable)
{
    ...
    else if((SDO_C->bufferSize > SDO_C->pst) && blockEnable != 0)
    { /*  BLOCK transfer */
...
/*  RX data ****************************************************************************************** */
    if(SDO_C->CANrxNew)
    {
        ...
        switch (SDO_C->state)
        {
            ...
            case SDO_STATE_BLOCKDOWNLOAD_INITIATE:
            { /* waiting on reply on block download initiated */
            ...
            case SDO_STATE_BLOCKDOWNLOAD_BLOCK_ACK:
            { /*  waiting block ACK */
                if (SCS == SCS_DOWNLOAD_BLOCK)
                {
    ...
    if(SDO_C->CANrxNew)
    {
        ...
        switch (SDO_C->state)
        {
            ...
            case SDO_STATE_BLOCKUPLOAD_INITIATE:
            {
                ...
                else if (SCS == SCS_UPLOAD_INITIATE)
                { /*  switch to regular segmented transfer */
                    if(SDO_C->CANrxData[0] & 0x02)
                    {
                        uint8_t size;
                        /* Expedited transfer */
...
/*  TMO *************************************************************************************************** */
    ...
    if(SDO_C->timeoutTimerBLOCK >= (SDOtimeoutTime/2))
    { /*  block TMO */
        SDO_C->state = SDO_STATE_BLOCKUPLOAD_BLOCK_ACK;
    }
