        ...
        uint16_t CANbitRate)
{
    ...
    CAN_InitStruct.CAN_SJW = CAN_SJW_4tq;     // changed by FARID, old value = CAN_SJW_1tq;
    CAN_InitStruct.CAN_BS1 = CAN_BS1_12tq;    // changed by FARID, old value = CAN_BS1_3tq;
    CAN_InitStruct.CAN_BS2 = CAN_BS2_5tq;     // changed by FARID, old value = CAN_BS2_2tq;
    CAN_InitStruct.CAN_NART = ENABLE;   // No Automatic retransmision

   /* CO - Changed FARID Start */
...
//    NVIC_Init(&NVIC_InitStructure);
   ...
   // CAN_OperatingModeRequest(CANmodule->CANbaseAddress, CAN_Mode_Normal); // Not needed as after init Can_init functions puts the controller in normal mode - FARID
    ...
    if(CANmodule->CANtxCount > 0)
    {
        ...
        for(i = CANmodule->txSize; i > 0; i--)
        {
            ...
            if(buffer->bufferFull)
            {
                buffer->bufferFull = 0;
                CANmodule->CANtxCount--;
    txBuff = getFreeTxBuff(CANmodule);    //FARID
...
static void CO_CANsendToModule(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer, uint8_t transmit_mailbox)
{
    ...
    /*Test code, FARID using Drivers Start*/
    ...
    /*Test code, FARID using Drivers End*/

}

/* CO- FARID Changed Start */
/******************************************************************************/
static void CO_CANClkSetting (void)
{
	/* Enable GPIO clock */
